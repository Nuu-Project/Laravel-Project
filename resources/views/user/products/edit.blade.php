...
<body class="font-body">
                            ...
                            <div class="grid gap-2">
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                    ...
                                    <div class="relative">
                                        ...
                                        <label for="image{{ $i }}" class="flex flex-col items-center justify-center w-full h-40 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                            <div id="placeholder{{ $i }}" class="flex flex-col items-center justify-center pt-5 pb-6 {{ $product->getMedia('images')->sortBy('order_column')->values()->get($i) ? 'hidden' : '' }}">
                                            ...
                                            <div id="preview{{ $i }}" class="absolute inset-0 flex items-center justify-center {{ $product->getMedia('images')->sortBy('order_column')->values()->get($i) ? '' : 'hidden' }}">
                                                <img src="{{ $product->getMedia('images')->sortBy('order_column')->values()->get($i)?->getUrl() ?? '#' }}" alt="預覽圖片" class="max-w-full max-h-full object-contain">
                                            </div>
                                        </label>
                                        <button type="button" class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1 m-1 {{ $product->getMedia('images')->sortBy('order_column')->values()->get($i) ? '' : 'hidden' }}" id="deleteButton{{ $i }}" onclick="deleteImage({{ $product->id }}, {{ $product->getMedia('images')->sortBy('order_column')->values()->get($i)?->id ?? 'null' }}, {{ $i }})">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        @for ($i = 0; $i < 5; $i++)
            ...
            const deleteButton{{ $i }} = document.getElementById('deleteButton{{ $i }}');
            if (preview{{ $i }}.querySelector('img')?.src && preview{{ $i }}.querySelector('img').src !== window.location.href + '#') {
