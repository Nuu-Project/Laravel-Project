<x-template-layout>
    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('user.products.chirps.store', ['product' => $product->id]) }}">
            @csrf
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            @if ($chirp->user->is(auth()->user()))
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('user.products.chirps.edit', [
                                            'product' => $product->id,
                                            'chirp' => $chirp->id,
                                        ])">
                                            {{ __('更改') }}
                                        ...
                                        <form method="POST"
                                            action="{{ route('user.products.chirps.destroy', ['product' => $product->id, 'chirp' => $chirp->id]) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('user.products.chirps.destroy', [
                                                'product' => $product->id,
                                                'chirp' => $chirp->id,
                                            ])"
