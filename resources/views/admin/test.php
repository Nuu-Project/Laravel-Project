...
<body class="font-body">
    <div class="flex flex-col md:flex-row min-h-screen bg-gray-100">
        ...
        <div class="w-full md:w-64 bg-white shadow-md">
            <div class="p-4 text-xl md:text-2xl font-bold">使用者後台</div>
            <nav class="mt-4">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
                        ...
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-3 py-2 border border-transparent text-2xl leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                    ...
                                    <div class="ml-1">
                                        <svg class="fill-current h-4 w-4" xmlns="h`tp://www.w3.org/2000/svg" viewBox="0 0 20 20">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    <h3 class="text-gray-700 text-2xl md:text-3xl font-medium mb-6">新增刊登商品</h3>
                    
                    <form class="mt-8 space-y-6" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="grid gap-4 md:gap-6">
                            <div class="grid gap-2">
                                <label class="text-sm font-medium leading-none" for="name">書名</label>
                                <input class="flex h-10 w-full rounded-md border px-3 py-2 text-sm" id="name" name="name" placeholder="請輸入書名" />
                            </div>
                            <div class="grid gap-2">
                                <label class="text-sm font-medium leading-none" for="price">價格</label>
                                <input class="flex h-10 w-full rounded-md border px-3 py-2 text-sm" id="price" name="price" placeholder="輸入價格" type="number" />
                            </div>
                            <div class="grid gap-2">
                                <label class="text-sm font-medium leading-none" for="grade">年級</label>
                                <select id="grade" name="grade" class="flex h-10 w-full rounded-md border px-3 py-2 text-sm">
                            ...
                            <div class="grid gap-2">
                                ...
                                <select id="semester" name="semester" class="flex h-10 w-full rounded-md border px-3 py-2 text-sm">
                            ...
                            <div class="grid gap-2">
                                ...
                                <select id="category" name="category" class="flex h-10 w-full rounded-md border px-3 py-2 text-sm">
                            ...
                            <div class="grid gap-2">
                                <label class="text-sm font-medium leading-none" for="description">商品介紹</label>
                                <textarea class="flex min-h-[80px] w-full rounded-md border px-3 py-2 text-sm" id="description" name="description" placeholder="請填寫有關該書的書況or使用情況等等~~" rows="4"></textarea>
                            </div>
                            <div class="grid gap-2">
                                <label class="text-sm font-medium leading-none">上傳圖片</label>
                                        ...
                                        <div class="relative w-full aspect-square">
                                            ...
                                            <label for="image{{ $i }}" class="flex flex-col items-center justify-center w-full h-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 overflow-hidden">
                                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                                    <p class="text-xs text-gray-500">圖片 {{ $i }}</p>
                                                </div>
                        ...
                        <button class="w-full inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-blue-500 text-white hover:bg-blue-700 h-10 px-4 py-2" type="submit">
                            刊登商品
                        </button>
                    </form>
                </div>
